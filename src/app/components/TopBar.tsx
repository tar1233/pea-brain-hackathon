    ...
    <header className="flex h-[72px] shrink-0 items-center gap-4 border-b border-gray-100/60 bg-white/60 px-4 md:px-8 backdrop-blur-2xl sticky top-0 z-50 shadow-[0_4px_30px_rgb(0,0,0,0.02)]">
      ...
      <div>
        <div className="flex items-center gap-2">
          <h1 className="text-[16.5px] font-bold text-[#5c2b86]">{current.title}</h1>
          {alertBadge ? (
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-critical-600 text-[16.5px] font-bold text-white">
              {alertBadge}
            </span>
          ) : null}
        </div>
        <div className="mt-0.5 text-[16.5px] text-[#8a94ab]">{current.subtitle}</div>
      </div>

      <div className="flex-1" />

      {/* Date */}
      <span className="text-[16.5px] text-[#8a94ab]">{dateStr} • {timeStr} น.</span>


      {/* Spacer */}
      <button
        onClick={() => setShowManual(true)}
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200/60 text-[16.5px] font-bold text-purple-700 hover:from-purple-100 hover:to-indigo-100 hover:border-purple-300 transition-all cursor-pointer shadow-sm hover:shadow group"
      ...
      <div className="flex items-center gap-2 cursor-pointer">
        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#d04ca6] to-[#9f2c80] border-2 border-[#f3d3e7] flex items-center justify-center text-[16.5px] font-bold text-white">
          {activeTab === "warehouse" ? "วญ" : "ขภ"}
        </div>
        <div className="hidden lg:block">
          <div className="text-[16.5px] font-semibold text-text-primary leading-tight">
            {activeTab === "warehouse" ? "คุณวรัญญู จันทร์ศิริ" : "คุณขวัญภิชา"}
          </div>
          <div className="text-[16.5px] text-text-muted">
  ...
  const ScreenshotPreview = ({ src, alt }: { src: string; alt: string }) => (
    <div className="rounded-xl border border-slate-200 overflow-hidden shadow-sm mt-3 mb-3">
      <div className="bg-slate-100 px-3 py-1.5 border-b border-slate-200 flex items-center gap-2">
        ...
        <span className="text-[16.5px] text-slate-500 font-medium">📸 ตัวอย่างหน้าจอ — {alt}</span>
      </div>
          ...
          <div className="rounded-2xl border-2 border-purple-200 bg-purple-50/50 overflow-hidden shadow-sm flex flex-col mb-2">
            <div className="px-4 py-3 border-b border-purple-100 flex items-center justify-between bg-white/50">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[16.5px] font-bold text-purple-900">สไลด์นำเสนอ (Pitch Deck)</span>
              </div>
              <a 
                href="/pitch.html" 
                target="_blank" 
                className="text-[16.5px] font-bold text-purple-600 hover:text-purple-800 bg-purple-100/50 hover:bg-purple-100 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"
              >
            ...
            <div className="px-4 py-2 bg-white/50 text-[16.5px] text-purple-700 flex justify-center items-center gap-2 border-t border-purple-100">
              💡 <span>คลิกในกรอบด้านบน แล้วใช้ปุ่ม <kbd className="px-1.5 py-0.5 bg-white border border-purple-200 rounded text-[16.5px] shadow-sm font-sans mx-0.5">→</kbd> หรือคลิกที่ขอบขวาเพื่อเปลี่ยนสไลด์</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={i} className="rounded-xl bg-gradient-to-br from-slate-50 to-purple-50/30 border border-slate-200/60 p-3">
                <div className="text-[16.5px] font-bold text-slate-800">{a.title}</div>
                <div className="text-[16.5px] text-slate-600 mt-0.5">{a.desc}</div>
              </div>
            ))}
          </div>
          <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-4 mt-3">
            <div className="text-[16.5px] font-bold text-emerald-800 mb-1">📊 ข้อมูลที่ผู้บริหารต้องรู้ (Executive Insights)</div>
            <div className="text-[16.5px] text-emerald-700 leading-relaxed">
    ...
    },
    {
      ...
      content: (
        <div className="space-y-3">
          <p className="text-[16.5px] text-slate-700">ระบบมี 4 เมนูหลักใน Sidebar:</p>
          {[
            ...
            <div key={i} className="flex gap-3 rounded-xl border border-slate-200 bg-white p-3 hover:border-purple-200 hover:bg-purple-50/20 transition-colors">
              ...
              <div>
                <div className="text-[16.5px] font-bold text-slate-800">{m.name}</div>
                <div className="text-[16.5px] text-slate-600 mt-0.5 leading-relaxed">{m.desc}</div>
              </div>
            </div>
          ))}
          <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 text-[16.5px] text-amber-800 mt-2">
            💡 <strong>บนมือถือ:</strong> กดปุ่ม ☰ มุมบนซ้ายเพื่อเปิด Sidebar
          </div>
          <div className="mt-4">
            <div className="text-[16.5px] font-bold text-slate-800 mb-2">🔧 ปุ่มด้านล่าง Sidebar</div>
            <div className="space-y-2">
              <div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50/50 p-3">
                <div className="text-[20px] shrink-0">💬</div>
                <div>
                  <div className="text-[16.5px] font-bold text-amber-800">โหมดเสนอแนะ</div>
                  <div className="text-[16.5px] text-amber-700 mt-0.5 leading-relaxed">
              ...
              <div className="flex gap-3 rounded-xl border border-red-200 bg-red-50/50 p-3">
                ...
                <div>
                  <div className="text-[16.5px] font-bold text-red-800">เคลียร์แผนทั้งหมด (Test)</div>
                  <div className="text-[16.5px] text-red-700 mt-0.5 leading-relaxed">
      ...
      content: (
        <div className="space-y-4">
          ...
          <div className="rounded-xl bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 p-4">
            <div className="text-[16.5px] font-bold text-indigo-800 mb-2">🎯 Hero Banner (ส่วนบนสุด)</div>
            <div className="text-[16.5px] text-indigo-700 leading-relaxed">
              แถบ Gradient สีม่วง-ทอง แสดงข้อมูลสรุปสำคัญ: มูลค่าความเสี่ยงรวม (VaR), สต็อกเฉลี่ย, ประสิทธิภาพ AI
            </div>
          </div>
          <div className="text-[16.5px] font-bold text-slate-800">📊 KPI Cards (5 การ์ด)</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[16.5px]">
          ...
          <div className="text-[16.5px] font-bold text-slate-800 mt-2">📋 ตารางสรุปแจ้งเตือนด่วน</div>
          <div className="text-[16.5px] text-slate-600 leading-relaxed">
            รายการที่อยู่ในตารางนี้คือ <strong>พัสดุวิกฤต (Critical)</strong> ที่ AI คัดกรองมาให้พิจารณาก่อน สามารถกดปุ่มสีแดง <span className="inline-block px-1.5 py-0.5 bg-red-600 text-white text-[16.5px] rounded">ให้ AI เข้าไปวิเคราะห์</span> ได้ทันที
          </div>
        </div>
      ),
    },
    {
      ...
      content: (
        <div className="space-y-4">
          ...
          <div className="text-[16.5px] text-slate-700 leading-relaxed">
          ...
          <div className="flex flex-col gap-2">
            {[
              ...
              <div key={i} className={`flex items-start gap-3 p-3 rounded-xl border ${r.bg}`}>
                <div className={`text-[16.5px] font-bold ${r.text} w-[130px] shrink-0`}>{r.level}</div>
                <div className={`text-[16.5px] ${r.text} leading-relaxed`}>{r.desc}</div>
              </div>
            ))}
          </div>
          <div className="mt-4 p-4 rounded-xl bg-purple-50 border border-purple-100">
            <div className="text-[16.5px] font-bold text-purple-800 mb-2">🤖 การทำงานของ AI Copilot</div>
            <ol className="list-decimal list-inside text-[16.5px] text-purple-800 leading-relaxed space-y-1.5">
      ...
      content: (
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 p-3 rounded-lg bg-slate-50 border border-slate-200 overflow-x-auto">
            <span className="text-[16.5px] font-bold text-emerald-600 bg-emerald-100 px-2 py-1 rounded">✅ วิเคราะห์</span>
            <span className="text-slate-400">→</span>
            <span className="text-[16.5px] font-bold text-emerald-600 bg-emerald-100 px-2 py-1 rounded">✅ อนุมัติแผน</span>
            <span className="text-slate-400">→</span>
            <span className="text-[16.5px] font-bold text-blue-600 bg-blue-100 px-2 py-1 rounded">🔵 จัดซื้อ (PO)</span>
            <span className="text-slate-400">→</span>
            <span className="text-[16.5px] font-bold text-slate-500 bg-slate-200 px-2 py-1 rounded">⬜ ส่งมอบ</span>
          </div>
          <div className="text-[16.5px] font-bold text-slate-800 mt-4 mb-2">ฟีเจอร์สำคัญ:</div>
          <ul className="space-y-2">
            {[
              ...
              <li key={i} className="flex items-start gap-2 text-[16.5px] text-slate-700 leading-relaxed">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-2">
            {[
              ...
              <div key={i} className="px-4 py-2.5 rounded-xl bg-purple-50 border border-purple-100 text-[16.5px] text-purple-800 font-medium italic">
                {q}
              </div>
            ))}
          </div>
          <div className="rounded-xl bg-slate-50 border border-slate-200 p-4 mt-2">
            <div className="text-[16.5px] font-bold text-slate-800 mb-1">💡 ความสามารถพิเศษ</div>
            <div className="text-[16.5px] text-slate-600 leading-relaxed">
      ...
      content: (
        <div className="space-y-6">
          <div className="rounded-xl bg-red-50 border border-red-200 p-4 relative overflow-hidden">
            ...
            <div className="text-[16.5px] font-bold text-red-800 mb-1">🚨 สถานการณ์: หม้อแปลง 160 kVA ใกล้หมดสต็อก!</div>
            <div className="text-[16.5px] text-red-700 font-medium">สต็อกเหลือ 12 เครื่อง • Demand 339/เดือน • ใช้ได้อีก 1 วัน!</div>
          </div>
          ...
          <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-purple-200 before:via-purple-100 before:to-transparent">
            {[
              ...
              <div key={i} className="relative flex items-start gap-4 md:gap-6">
                <div className="sticky top-4 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-purple-600 text-white font-bold text-[16.5px] shadow-sm z-10">
                  {s.step}
                </div>
                <div className="flex-1 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:border-purple-200 transition-colors">
                  <h4 className="text-[16.5px] font-bold text-slate-900">{s.title}</h4>
                  <p className="mt-1 text-[16.5px] text-slate-600 leading-relaxed">{s.desc}</p>
                  <div className="text-[16.5px] text-purple-600 font-bold mt-2 mb-2">👆 {s.action}</div>
                  {s.img && (
      ...
      <div className="w-full max-w-[1100px] max-h-[96vh] bg-white rounded-[24px] shadow-[0_25px_60px_rgba(0,0,0,0.25)] border border-slate-100 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-gradient-to-r from-purple-50 to-indigo-50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[16.5px] font-extrabold text-slate-900 tracking-tight">📘 คู่มือการใช้งาน PEA Brain</h2>
              <p className="text-[16.5px] text-slate-500 font-medium">ระบบสนับสนุนการตัดสินใจจัดซื้อพัสดุอัจฉริยะ</p>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row flex-1 overflow-hidden min-h-0">
            ...
            <div className="p-2 space-y-0.5">
              {sections.map((s, i) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded-xl text-[16.5px] font-medium transition-all cursor-pointer ${
                    activeSection === i
          ...
          <div className="sm:hidden flex overflow-x-auto border-b border-slate-100 bg-slate-50 shrink-0">
            {sections.map((s, i) => (
              <button
                key={s.id}
                onClick={() => setActiveSection(i)}
                className={`px-3 py-2 text-[16.5px] whitespace-nowrap font-medium border-b-2 transition-colors ${
          ...
          <div className="flex-1 overflow-y-auto p-5 md:p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-[16.5px] font-extrabold text-slate-900">{sections[activeSection].title}</h3>
            </div>
