        ...
        <div className={`max-w-[1400px] mx-auto px-8 ${embedded ? 'pt-6 pb-6' : 'pt-6 pb-8'}`}>
          ...
          <div className="flex items-start justify-between gap-8">
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-1.5 text-[13px] font-bold tracking-widest text-white/80 uppercase mb-3">
                <Sparkles size={13} />
            ...
            <div className="flex gap-3">
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-center min-w-[110px]">
                <div className="text-[13px] text-white/50 font-bold uppercase tracking-wider mb-0.5">สต็อกเหลือ</div>
                <div className={`text-[30px] font-black leading-none ${stockPercent < 30 ? 'text-red-300' : 'text-white'}`}>{daysOfStock}</div>
                <div className="text-[13px] text-white/50 font-medium mt-0.5">วัน</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-center min-w-[110px]">
                <div className="text-[13px] text-white/50 font-bold uppercase tracking-wider mb-0.5">Safety Stock</div>
                <div className={`text-[30px] font-black leading-none ${stockPercent < 30 ? 'text-red-300' : 'text-amber-300'}`}>{stockPercent}%</div>
                <div className="text-[13px] text-white/50 font-medium mt-0.5">ของเกณฑ์</div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 text-center min-w-[130px]">
                <div className="text-[13px] text-white/50 font-bold uppercase tracking-wider mb-0.5">ราคาต่อหน่วย</div>
                <div className="text-[24px] font-black leading-none">{formatCurrency(material?.unitPrice || 0)}</div>
                <div className="text-[13px] text-white/50 font-medium mt-0.5">ราคากลาง: {formatCurrency(material?.budgetPrice || 0)}</div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="grid gap-6 lg:grid-cols-5">
              ...
              <div className="lg:col-span-3 rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-[13px] text-slate-400 mt-1">ต้นทุนสินค้า + Holding Cost (อัตรา 20% ต่อปี) • ซื้อทีเดียว vs ทยอยซื้อ</p>
                  </div>
                  <div className="rounded-full bg-emerald-50 px-3 py-1 text-[13px] font-bold text-emerald-700 flex items-center gap-1.5 border border-emerald-100">
                    ...
                    <div className="space-y-5 mt-2">
                      ...
                      <div>
                        ...
                        <div className="w-full bg-slate-100 rounded-full h-10 overflow-hidden relative">
                          ...
                          >
                            <span className="text-[13px] font-bold text-white drop-shadow">Holding Cost ฿{singleHC.toLocaleString()}</span>
                          </div>
                        </div>
                        <div className="flex gap-2 mt-1 text-[13px] text-slate-400">
                      ...
                      <div>
                        ...
                        <div className="w-full bg-slate-100 rounded-full h-10 overflow-hidden relative">
                          ...
                          >
                            <span className="text-[13px] font-bold text-white drop-shadow">Holding Cost ฿{phasedHC.toLocaleString()}</span>
                          </div>
                        </div>
                        <div className="flex gap-2 mt-1 text-[13px] text-slate-400">
                          <span>ต้นทุนสินค้า ฿{goodsCost.toLocaleString()}</span>
                      ...
                      <div className="rounded-xl bg-gradient-to-r from-emerald-50 to-green-50 border-2 border-emerald-200 p-4 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-[13px] font-bold text-emerald-800 uppercase tracking-wider">ประหยัดได้จากการทยอยซื้อ</div>
                            <div className="text-[13px] text-emerald-600 mt-0.5">ลด Holding Cost จากสต็อกเฉลี่ย 50% → 20%</div>
                          </div>
                        </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="rounded-2xl bg-white p-5 shadow-sm border border-slate-200">
                  <div className="text-[13px] font-bold text-blue-600 uppercase tracking-wider mb-2 flex items-center gap-1.5"><Target size={13} /> ทำไมต้องสั่ง? (Demand)</div>
                  <p className="text-[13px] text-slate-700 leading-relaxed font-medium">{aiResult.demandAnalysis}</p>
                </div>
                <div className="rounded-2xl bg-white p-5 shadow-sm border border-slate-200">
                  <div className="text-[13px] font-bold text-amber-600 uppercase tracking-wider mb-2 flex items-center gap-1.5"><BarChart3 size={13} /> เปิดประมูลเมื่อไหร่? (Market)</div>
                  <p className="text-[13px] text-slate-700 leading-relaxed font-medium">{aiResult.marketAnalysis}</p>
                </div>
                <div className="rounded-2xl bg-white p-5 shadow-sm border border-slate-200">
                  <div className="text-[13px] font-bold text-red-600 uppercase tracking-wider mb-2 flex items-center gap-1.5"><Zap size={13} /> ต้องเริ่มสั่งเมื่อไหร่? (Supplier)</div>
                ...
                <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="rounded-xl bg-red-50 border border-red-100 p-4 text-center">
                      <div className="text-[13px] font-bold text-red-600 uppercase tracking-wider mb-2">🔴 ซื้อทีเดียว (สต็อกเฉลี่ย 50%)</div>
                      <div className="text-[26px] font-black text-red-700">฿{singleHC.toLocaleString()}</div>
                      <div className="text-[13px] text-red-400 mt-1">Holding Cost ต่อปี</div>
                    </div>
                    <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-4 text-center">
                      <div className="text-[13px] font-bold text-emerald-600 uppercase tracking-wider mb-2">🟢 ทยอยซื้อ 4 รอบ (สต็อกเฉลี่ย 20%)</div>
                      <div className="text-[26px] font-black text-emerald-700">฿{phasedHC.toLocaleString()}</div>
                      <div className="text-[13px] text-emerald-400 mt-1">Holding Cost ต่อปี</div>
                    </div>
                    <div className="rounded-xl bg-gradient-to-br from-amber-50 to-yellow-50 border-2 border-amber-200 p-4 text-center">
                      <div className="text-[13px] font-bold text-amber-600 uppercase tracking-wider mb-2">💰 ประหยัดได้</div>
                      <div className="text-[26px] font-black text-amber-700">฿{savings.toLocaleString()}</div>
                      <div className="text-[13px] text-amber-500 mt-1">ต่อปี จากการลดสต็อก</div>
                    </div>
                  </div>
                </div>
              );
            ...
            {!readonly && (
              ...
              <div className="rounded-2xl border-2 border-emerald-400 bg-white p-6 relative overflow-hidden shadow-sm hover:shadow-md transition-all">
                <div className="absolute top-0 right-0 bg-emerald-500 text-white px-4 py-1.5 text-[13px] font-bold rounded-bl-2xl flex items-center gap-1.5">
                  <CheckCircle2 size={12} /> AI แนะนำ
                </div>
                <h3 className="text-[18px] font-bold text-emerald-800 mb-1">Plan A: {aiResult.planA.title}</h3>
                <div className="text-[13px] text-slate-400 mb-4">จำนวน: {aiResult.planA.qty.toLocaleString()} {material?.unit} • {formatCurrency(aiResult.planA.qty * (material?.unitPrice || 0))}</div>
                
                <div className="space-y-3">
                  <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-3">
                    <div className="text-[13px] font-bold text-emerald-700 uppercase tracking-wider mb-1">🔮 ถ้าเลือกแผนนี้ อนาคตจะเป็นยังไง?</div>
                    <p className="text-[12px] text-slate-700 leading-relaxed">{aiResult.planA.futureImpact}</p>
                  </div>
                  <div className="rounded-xl bg-blue-50 border border-blue-100 p-3">
                    <div className="text-[13px] font-bold text-blue-700 uppercase tracking-wider mb-1">📦 สต็อกอยู่ได้อีกกี่เดือน? ต้องสั่งเพิ่มอีกไหม?</div>
                    <p className="text-[12px] text-slate-700 leading-relaxed">{aiResult.planA.supplyForecast}</p>
                  </div>
                  <div className="rounded-xl bg-indigo-50 border border-indigo-100 p-3">
                    <div className="text-[13px] font-bold text-indigo-700 uppercase tracking-wider mb-1">💰 วิเคราะห์ต้นทุน & ความคุ้มค่า</div>
                    <p className="text-[12px] text-slate-700 leading-relaxed">{aiResult.planA.costAnalysis}</p>
                  </div>
                  <div className="rounded-xl bg-amber-50 border border-amber-200 p-3">
                    <div className="text-[13px] font-bold text-amber-700 uppercase tracking-wider mb-1 flex items-center gap-1"><AlertTriangle size={13} /> สถานการณ์เลวร้ายที่อาจเกิด</div>
                    <p className="text-[12px] text-amber-900 leading-relaxed">{aiResult.planA.riskScenarios}</p>
                  </div>
                  <div className="rounded-xl bg-purple-50 border border-purple-100 p-3">
                    <div className="text-[13px] font-bold text-purple-700 uppercase tracking-wider mb-1">🛡️ วิธีรับมือเมื่อเกิดความเสี่ยง</div>
                  ...
                  <div className="rounded-xl bg-slate-100 border border-slate-200 p-3">
                    <div className="text-[13px] font-bold text-slate-700 uppercase tracking-wider mb-1">✅ ปัญหาจบไหม? ต้องทำอะไรต่อ?</div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 relative overflow-hidden shadow-sm hover:shadow-md hover:border-slate-300 transition-all">
                ...
                <div className="text-[13px] text-slate-400 mb-4">จำนวน: {aiResult.planB.qty.toLocaleString()} {material?.unit} • {formatCurrency(aiResult.planB.qty * (material?.unitPrice || 0))}</div>
                
                <div className="space-y-3">
                  <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
                    <div className="text-[13px] font-bold text-slate-500 uppercase tracking-wider mb-1">🔮 ถ้าเลือกแผนนี้ อนาคตจะเป็นยังไง?</div>
                    <p className="text-[12px] text-slate-600 leading-relaxed">{aiResult.planB.futureImpact}</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
                    <div className="text-[13px] font-bold text-slate-500 uppercase tracking-wider mb-1">📦 สต็อกอยู่ได้อีกกี่เดือน? ต้องสั่งเพิ่มอีกไหม?</div>
                    <p className="text-[12px] text-slate-600 leading-relaxed">{aiResult.planB.supplyForecast}</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
                    <div className="text-[13px] font-bold text-slate-500 uppercase tracking-wider mb-1">💰 วิเคราะห์ต้นทุน & ความคุ้มค่า</div>
                    <p className="text-[12px] text-slate-600 leading-relaxed">{aiResult.planB.costAnalysis}</p>
                  </div>
                  <div className="rounded-xl bg-red-50 border border-red-200 p-3">
                    <div className="text-[13px] font-bold text-red-700 uppercase tracking-wider mb-1 flex items-center gap-1"><ShieldAlert size={13} /> สถานการณ์เลวร้ายที่อาจเกิด</div>
                    <p className="text-[12px] text-red-900 leading-relaxed">{aiResult.planB.riskScenarios}</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
                    <div className="text-[13px] font-bold text-slate-500 uppercase tracking-wider mb-1">🛡️ วิธีรับมือเมื่อเกิดความเสี่ยง</div>
                    <p className="text-[12px] text-slate-600 leading-relaxed">{aiResult.planB.mitigation}</p>
                  </div>
                  <div className="rounded-xl bg-slate-100 border border-slate-200 p-3">
                    <div className="text-[13px] font-bold text-slate-600 uppercase tracking-wider mb-1">✅ ปัญหาจบไหม? ต้องทำอะไรต่อ?</div>
            ...
            {aiResult.raw && (
              <details className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                <summary className="px-6 py-4 text-[12px] font-bold text-slate-400 cursor-pointer hover:bg-slate-50 transition flex items-center gap-2">
                  ...
                  {!isAnalyzing && (
                    <button onClick={(e) => { e.preventDefault(); hasCalledRef.current = false; callAI(); }} className="ml-auto text-purple-600 hover:text-purple-800 flex items-center gap-1 text-[13px] font-bold">
                      <RefreshCw size={12} /> วิเคราะห์ใหม่
                    </button>
                  )}
                </summary>
                <div className="px-6 pb-6 text-[13px] text-slate-600 whitespace-pre-wrap leading-relaxed max-h-[400px] overflow-y-auto font-mono border-t border-slate-100 pt-4">{aiResult.raw}</div>
              </details>
            )}
          </div>
        )}
