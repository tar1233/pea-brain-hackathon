...
export default function Sidebar({ activeTab, setActiveTab }: SidebarProps) {
  ...
  return (
    <aside className={`${isCollapsed ? "w-[80px]" : "w-[260px]"} shrink-0 sticky top-0 h-screen flex flex-col relative overflow-hidden transition-all duration-300 ease-in-out border-r border-white/10 shadow-[12px_0_40px_rgba(83,0,93,0.15)] z-[10000]`}
      ...
      {/* Logo */}
      <div className={`pt-6 pb-5 relative z-10 flex items-center ${isCollapsed ? "px-2 justify-center" : "px-5"}`}>
        <div className="flex items-center gap-3">
          <img
            src="/pea-official-logo.png"
            alt="PEA Logo"
            className="w-10 h-10 shrink-0 object-contain drop-shadow-[0_0_8px_rgba(255,255,255,0.4)]"
          />
          {!isCollapsed && (
      ...
      {/* AI Copilot CTA */}
      <div className={`mx-3 mb-5 rounded-2xl border border-white/12 cursor-pointer transition-all hover:border-white/25 relative z-10 shadow-[0_18px_35px_rgba(46,0,66,0.22)] ${isCollapsed ? "p-3 flex justify-center" : "p-4"}`}
        ...
        {menuItems.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id)} title={isCollapsed ? item.label : undefined}
              ...
              <Icon size={18} className={`shrink-0 ${isActive ? "text-[#EDC878] drop-shadow-md" : "text-white/70"}`} />
      ...
      <div className="px-3 pb-10 pt-2 relative z-10 flex flex-col gap-2">
        ...
        >
          ...
          {!isCollapsed && <span className="truncate">{isFeedbackOn ? "ปิดโหมดเสนอแนะ" : "โหมดเสนอแนะ"}</span>}
        </button>
        <button
          onClick={async () => {
