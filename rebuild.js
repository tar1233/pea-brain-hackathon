const fs = require('fs');
const content = fs.readFileSync('src/app/components/TopBar.tsx', 'utf8');
...
function ManualModal({ onClose }: { onClose: () => void }) {
  ...
  const ScreenshotPreview = ({ src, alt }: { src: string; alt: string }) => (
    <div className="rounded-xl border border-slate-200 overflow-hidden shadow-sm mt-3 mb-3 bg-slate-50">
      ...
      <div className="flex justify-center p-4">
        <img src={src} alt={alt} className="max-w-full max-h-[400px] object-contain rounded-lg shadow-sm border border-slate-200/50 bg-white" loading="lazy" />
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-[20000] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-2 md:pr-4 md:py-4 md:pl-[276px] animate-in fade-in duration-200">
      <div className="w-full max-w-[1100px] max-h-[96vh] bg-white rounded-[24px] shadow-[0_25px_60px_rgba(0,0,0,0.25)] border border-slate-100 flex flex-col overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row flex-1 overflow-hidden min-h-0">
          {/* Sidebar */}
          <div className="w-[180px] md:w-[200px] border-r border-slate-100 bg-slate-50/50 overflow-y-auto shrink-0 hidden sm:block">
...
fs.writeFileSync('src/app/components/TopBar.tsx', newContent);
